$PROB MAB IN MONKEY, 2COMP + FULL TMDD (KSYN/KDEG/KON/KOFF/KINT), FO   P:ROOT  F:CL~BWT,V1~BWT,KSYN~BWT
...
$PK
  ...
  KOFF = THETA(6)
  KINT = THETA(7)
  TVSY = THETA(8) + THETA(12) * BWT       ; target synthesis grows with body size
  TVDE = THETA(9)
  KSYN = TVSY * EXP(ETA(4))
  KDEG = TVDE * EXP(ETA(5))
...
$THETA
  ...
  (1E-4, 0.06, 0.6)       ; KSYN intercept (mg/h)
  (1E-3, 0.2, 2)          ; KDEG (1/h)
