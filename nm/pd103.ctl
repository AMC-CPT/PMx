$PROB NRS PD, COMBINED ERROR INSTEAD OF BOUNDED   P:pd100  F:BASE
...
$PRED
  ...
  IF (DVID.EQ.2) W = SQRT(THETA(9)**2 + THETA(10)**2 * IPRE**2)
  IF (W.LT.1E-6) W = 1E-6
  IRES = DV - IPRE
  IWRE = IRES / W
  Y    = IPRE + W * EPS(1)
...
$THETA
  ...
  (0, 0.5)     ; NRS additive error SD
  (0, 0.1)     ; NRS proportional error CV
